...
testParamConvNull TestEnv{..} = TestCase $ do
  ...
  [Solo r] <- (select_ conn "SELECT NULL") :: IO [Solo Null]
  execute conn "INSERT INTO nulltype (id, t1) VALUES (?,?)" (one, r)
  [Solo mr1] <- select_ conn "SELECT t1 FROM nulltype WHERE id = 1" :: IO [Solo (Maybe String)]
  assertEqual "nulls" Nothing mr1
  execute conn "INSERT INTO nulltype (id, t1) VALUES (?,?)" (two, "foo" :: String)
  [Solo mr2] <- select_ conn "SELECT t1 FROM nulltype WHERE id = 2" :: IO [Solo (Maybe String)]
  assertEqual "nulls" (Just "foo") mr2

testParamConvInt :: TestEnv -> Test
testParamConvInt TestEnv{..} = TestCase $ do
  [Solo r] <- (select conn "SELECT ?" (Solo one)) :: IO [Solo Int]
  assertEqual "value" 1 r
  [Solo r] <- (select conn "SELECT ?" (Solo one)) :: IO [Solo Integer]
  assertEqual "value" 1 r
  [Solo r] <- (select conn "SELECT ?+?" (one, two)) :: IO [Solo Int]
  assertEqual "value" 3 r
  [Solo r] <- (select conn "SELECT ?+?" (one, 15 :: Int64)) :: IO [Solo Int]
  assertEqual "value" 16 r
  [Solo r] <- (select conn "SELECT ?+?" (two, 14 :: Int32)) :: IO [Solo Int]
  assertEqual "value" 16 r
  [Solo r] <- (select conn "SELECT ?+?" (two, 14 :: Integer)) :: IO [Solo Int]
  assertEqual "value" 16 r
  -- This overflows 32-bit ints, verify that we get more than 32-bits out
  [Solo r] <- (select conn "SELECT 255*?" (Solo (0x7FFFFFFF :: Int32))) :: IO [Solo Int64]
  assertEqual "> 32-bit result"
    (255*0x7FFFFFFF :: Int64) (fromIntegral r)
  [Solo r] <- (select conn "SELECT 2*?" (Solo (0x7FFFFFFFFF :: Int64))) :: IO [Solo Int64]
  ...
  [Solo r] <- (select_ conn "SELECT 3") :: IO [Solo (Maybe Int)]
  assertEqual "should see Just 3" (Just 3) r
  [Solo r] <- (select conn "SELECT ?") (Solo (Nothing :: Maybe Int)) :: IO [Solo (Maybe Int)]
  assertEqual "should see nothing" Nothing r
  [Solo r] <- (select conn "SELECT ?") (Solo (Just three :: Maybe Int)) :: IO [Solo (Maybe Int)]
  assertEqual "should see 4" (Just 3) r

testParamConvIntWidths :: TestEnv -> Test
testParamConvIntWidths TestEnv{..} = TestCase $ do
  [Solo r] <- (select conn "SELECT ?" (Solo (1 :: Int8))) :: IO [Solo Int]
  assertEqual "value" 1 r
  [Solo r] <- (select conn "SELECT ?" (Solo (257 :: Int8))) :: IO [Solo Int] -- wrap around
  assertEqual "value" 1 r
  [Solo r] <- (select conn "SELECT ?" (Solo (257 :: Int16))) :: IO [Solo Int]
  assertEqual "value" 257 r
  [Solo r] <- (select conn "SELECT ?" (Solo (258 :: Int32))) :: IO [Solo Int]
  assertEqual "value" 258 r
  [Solo r] <- (select conn "SELECT ?" (Solo (1 :: Word8))) :: IO [Solo Int]
  assertEqual "value" 1 r
  [Solo r] <- (select conn "SELECT ?" (Solo (257 :: Word8))) :: IO [Solo Int] -- wrap around
  assertEqual "value" 1 r
  [Solo r] <- (select conn "SELECT ?" (Solo (257 :: Word16))) :: IO [Solo Int]
  assertEqual "value" 257 r
  [Solo r] <- (select conn "SELECT ?" (Solo (257 :: Word32))) :: IO [Solo Int]
  assertEqual "value" 257 r
  [Solo r] <- (select conn "SELECT ?" (Solo (0x100000000 :: Word64))) :: IO [Solo Int]
  assertEqual "value" 0x100000000 r
  [Solo r] <- (select conn "SELECT ?" (Solo (1 :: Integer))) :: IO [Solo Int]
  assertEqual "value" 1 r
  [Solo r] <- (select conn "SELECT ?" (Solo (1 :: Word))) :: IO [Solo Int]
  assertEqual "value" 1 r

testParamConvIntWidthsFromField :: TestEnv -> Test
testParamConvIntWidthsFromField TestEnv{..} = TestCase $ do
  [Solo r] <- (select conn "SELECT ?" (Solo (1 :: Int))) :: IO [Solo Int8]
  assertEqual "value" 1 r
  [Solo r] <- (select conn "SELECT ?" (Solo (257 :: Int))) :: IO [Solo Int8] -- wrap around
  assertEqual "value" 1 r
  [Solo r] <- (select conn "SELECT ?" (Solo (65536 :: Int))) :: IO [Solo Int16] -- wrap around
  assertEqual "value" 0 r
  [Solo r] <- (select conn "SELECT ?" (Solo (65536 :: Int))) :: IO [Solo Int32] -- wrap around
  assertEqual "value" 65536 r
  [Solo r] <- (select conn "SELECT ?" (Solo (258 :: Int))) :: IO [Solo Int32]
  assertEqual "value" 258 r
  [Solo r] <- (select conn "SELECT ?" (Solo (1 :: Int))) :: IO [Solo Word8]
  assertEqual "value" 1 r
  [Solo r] <- (select conn "SELECT ?" (Solo (257 :: Int))) :: IO [Solo Word8] -- wrap around
  assertEqual "value" 1 r
  [Solo r] <- (select conn "SELECT ?" (Solo (257 :: Int))) :: IO [Solo Word16]
  assertEqual "value" 257 r
  [Solo r] <- (select conn "SELECT ?" (Solo (257 :: Int))) :: IO [Solo Word32]
  assertEqual "value" 257 r
  [Solo r] <- (select conn "SELECT ?" (Solo (0x100000000 :: Int64))) :: IO [Solo Word64]
  assertEqual "value" 0x100000000 r
  [Solo r] <- (select conn "SELECT ?" (Solo (1 :: Int))) :: IO [Solo Word]
  assertEqual "value" 1 r

testParamConvFloat :: TestEnv -> Test
testParamConvFloat TestEnv{..} = TestCase $ do
  [Solo r] <- select conn "SELECT ?" (Solo (1.0 :: Double)) :: IO [Solo Double]
  assertEqual "value" 1.0 r
  [Solo r] <- select conn "SELECT ?*0.25" (Solo (8.0 :: Double)) :: IO [Solo Double]
...
testParamConvBools TestEnv{..} = TestCase $ do
  ...
  [Solo r1, Solo r2] <- select_ conn "SELECT b from bt" :: IO [Solo Bool]
...
testParamConvToRow TestEnv{..} = TestCase $ do
  [Solo (s :: Int)] <- select conn "SELECT 13" ()
  13 @=? s
  [Solo (s :: Int)] <- select conn "SELECT ?" (Solo one)
  1 @=? s
  [Solo (s :: Int)] <- select conn "SELECT ?+?" (one, two)
  (1+2) @=? s
  [Solo (s :: Int)] <- select conn "SELECT ?+?+?" (one, two, three)
  (1+2+3) @=? s
  [Solo (s :: Int)] <- select conn "SELECT ?+?+?+?" (one, two, three, 4 :: Int)
  (1+2+3+4) @=? s
  [Solo (s :: Int)] <- select conn "SELECT ?+?+?+?+?" (one, two, three, 4 :: Int, 5 :: Int)
  (1+2+3+4+5) @=? s
  [Solo (s :: Int)] <- select conn "SELECT ?+?+?+?+?+?" (one, two, three, 4 :: Int, 5 :: Int, 6 :: Int)
  (1+2+3+4+5+6) @=? s
  [Solo (s :: Int)] <- select conn "SELECT ?+?+?+?+?+?+?"
...
testParamNamed TestEnv{..} = TestCase $ do
  [Solo t1] <- selectNamed conn "SELECT :foo / :bar" [":foo" := two, ":bar" := one]
  t1 @=? (2 :: Int)
  ...
  [Solo t1] <- select_ conn "SELECT b FROM np"
  True @=? t1

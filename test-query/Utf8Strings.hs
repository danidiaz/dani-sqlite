...
testUtf8Simplest TestEnv{..} = TestCase $ do
  ...
  [Solo t1] <- query conn "SELECT t FROM utf WHERE id = ?" (Solo (1 :: Int))
  assertEqual "utf8" ("ääöö" :: String) t1
  [Solo t2] <- query conn "SELECT t FROM utf WHERE id = ?" (Solo (2 :: Int))
...
testBlobs TestEnv{..} = TestCase $ do
  ...
  [Solo t1] <- query conn "SELECT b FROM blobs WHERE id = ?" (Solo (1 :: Int))
